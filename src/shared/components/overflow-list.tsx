...
import { getOverflowListLayout } from '../utils/overflow-list-layout'
  ...
  return (
    <div className="relative min-w-0">
      ...
      >
        ...
        {layout.hiddenCount > 0 ? getOverflowItemRef.current(layout.hiddenCount) : null}
      </div>
      <div
        ...
        {items.map((_, index) => {
          ...
          return (
            <span key={`overflow-${hiddenCount}`} data-overflow-list-overflow={hiddenCount}>
              {getOverflowItemRef.current(hiddenCount)}
            </span>
          )
        })}
      </div>
