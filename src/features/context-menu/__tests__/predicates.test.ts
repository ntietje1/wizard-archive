...
describe('isType / isNotType', () => {
  it('isType matches correct type', () => {
    expect(p.isType(SIDEBAR_ITEM_TYPES.notes)(ctx({ item: createNote() }))).toBe(true)
    expect(p.isType(SIDEBAR_ITEM_TYPES.folders)(ctx({ item: createNote() }))).toBe(false)
  })

  it('isType matches any of multiple types', () => {
    expect(
      p.isType(SIDEBAR_ITEM_TYPES.notes, SIDEBAR_ITEM_TYPES.folders)(ctx({ item: createNote() })),
    ).toBe(true)
  })
  ...
  it('isNotType is inverse of isType', () => {
    expect(p.isNotType(SIDEBAR_ITEM_TYPES.folders)(ctx({ item: createNote() }))).toBe(true)
    expect(p.isNotType(SIDEBAR_ITEM_TYPES.notes)(ctx({ item: createNote() }))).toBe(false)
  })
...
describe('permission predicates', () => {
  ...
  it('hasEditAccess includes EDIT and higher', () => {
    ...
    expect(p.hasEditAccess(ctx({ permissionLevel: PERMISSION_LEVEL.FULL_ACCESS }))).toBe(true)
  })

  it('hasFullAccess requires FULL_ACCESS', () => {
    expect(p.hasFullAccess(ctx({ permissionLevel: PERMISSION_LEVEL.EDIT }))).toBe(false)
    expect(p.hasFullAccess(ctx({ permissionLevel: PERMISSION_LEVEL.FULL_ACCESS }))).toBe(true)
  })
})
