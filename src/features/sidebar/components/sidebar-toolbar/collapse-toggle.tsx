...
export function CollapseToggle() {
  const { visible, setVisible } = usePanelPreference(LEFT_SIDEBAR_PANEL_ID, LEFT_SIDEBAR_DEFAULTS)
  ...
  return (
    ...
    >
      {visible ? <PanelLeft className="h-4 w-4" /> : <PanelLeftOpen className="h-4 w-4" />}
    </Button>
  )
}
