...
describe('planDuplicateOperations', () => {
  it('returns no operations for empty inputs', () => {
    expect(
      planDuplicateOperations({
        items: [],
        targetParentId: null,
        targetItems: [],
      }),
  ...
  it('maps replace to replace for files and mergeFolder for folders', () => {
    ...
    const folderResult = planDuplicateOperations({
  ...
  it('supports skip and cancel decisions', () => {
    const skipResult = planDuplicateOperations({
      items: [item('note-1', 'Scene')],
      targetParentId: null,
      targetItems: [item('note-2', 'Scene')],
      decisions: decisions({ 'note-1': { action: 'skip' } }),
    })
    const cancelResult = planDuplicateOperations({
      items: [item('note-1', 'Scene')],
      targetParentId: null,
      targetItems: [item('note-2', 'Scene')],
      decisions: decisions({ 'note-1': { action: 'cancel' } }),
    })

    expect(skipResult).toMatchObject({ status: 'ready', operations: [] })
    expect(cancelResult).toMatchObject({ status: 'cancelled', operations: [] })
  })
