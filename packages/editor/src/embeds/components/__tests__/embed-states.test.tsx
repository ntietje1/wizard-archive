...
describe('embed states', () => {
  ...
  it('uses the shared successful drop target chrome while an item is dragged over it', () => {
    ...
    expect(screen.getByTestId('embed-empty-state')).toHaveAttribute(
      'data-drop-target-chrome',
      'item',
    )
  })
  ...
  it('does not apply drop target chrome in readonly mode', () => {
    render(
      <EmbedEmptyState
        mode="readonly"
        dropVisualState={{ isDropTarget: true, isFileDropTarget: false }}
      />,
    )
  ...
  it('renders recursive unavailable state', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent(/recursive embed/i)
  })
  ...
  it('offers request access for permission unavailable embeds', async () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent("isn't shared")
