...
import { clearStrokePathCache, getCachedStrokeDetailPath } from './stroke-path-cache'
import type { Bounds } from '../../utils/canvas-geometry-utils'
import { useCanvasNodeVisualSelection } from '../shared/use-canvas-node-visual-selection'
import { getStrokeHighlightPathSize, getStrokeSelectionPadding } from './stroke-node-interactions'
...
import type { CanvasRegisteredStrokeNodePaths } from '../../system/canvas-dom-registry'
...
export function StrokeNode({
  ...
  width,
  height,
}: CanvasNodeComponentProps<'stroke'>) {
  ...
  const isErasing = useStore(localOverlayStore, (state) => state.eraseErasingStrokeIds.has(id))
  ...
  const highlightSize = resolveCanvasScreenMinimumStrokeWidth(
    getStrokeHighlightPathSize(size, zoom),
    zoom,
  )
  const highlightD = visuallySelected ? getCachedStrokeDetailPath(id, data, highlightSize) : null
  const registeredStrokePathsRef = useRef<CanvasRegisteredStrokeNodePaths>({
    path: null,
    highlightPath: null,
    data,
  })
  registeredStrokePathsRef.current.data = data
  const unregisterStrokePathsRef = useRef<(() => void) | null>(null)
  const syncStrokePathRegistrationRef = useRef(() => undefined)
  syncStrokePathRegistrationRef.current = () => {
    const paths = registeredStrokePathsRef.current
    if (!paths.path && !paths.highlightPath) {
      unregisterStrokePathsRef.current?.()
      unregisterStrokePathsRef.current = null
      return
    }

    unregisterStrokePathsRef.current ??= domRuntime.registerStrokeNodePaths(id, paths)
  }
  const setStrokePathRef = useRef((element: SVGPathElement | null) => {
    registeredStrokePathsRef.current.path = element
    syncStrokePathRegistrationRef.current()
  }).current
  const setHighlightPathRef = useRef((element: SVGPathElement | null) => {
    registeredStrokePathsRef.current.highlightPath = element
    syncStrokePathRegistrationRef.current()
  }).current
  useEffect(() => {
    return () => clearStrokePathCache(id)
  }, [id])
  ...
  return (
    ...
    >
      ...
      <StrokeVisual
        ...
        pathRef={setStrokePathRef}
        highlightPathRef={setHighlightPathRef}
      />
    </ResizableNodeWrapper>
  )
}
