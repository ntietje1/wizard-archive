...
describe('ResizableNodeWrapper', () => {
  ...
  it('keeps horizontal resize zones usable on tiny horizontal-only nodes', () => {
    renderSelectionResize({
      resizeAxes: 'horizontal',
      nodes: [createNode('node-1', { x: 10, y: 20 }, 10, 10)],
    })

    expect(screen.getByTestId('canvas-selection-resize-zone-left')).toHaveStyle({
      bottom: '0px',
      left: '-9px',
      top: '0px',
      width: '18px',
    })
    expect(screen.getByTestId('canvas-selection-resize-zone-right')).toHaveStyle({
      bottom: '0px',
      right: '-9px',
      top: '0px',
      width: '18px',
  ...
  it('keeps vertical resize zones usable on tiny vertical-only nodes', () => {
    renderSelectionResize({
      resizeAxes: 'vertical',
      nodes: [createNode('node-1', { x: 10, y: 20 }, 10, 10)],
    })
    ...
    expect(screen.getByTestId('canvas-selection-resize-zone-bottom')).toHaveStyle({
      bottom: '-9px',
      height: '18px',
      left: '0px',
      right: '0px',
  ...
  it('ignores invalid locked aspect ratios during selection resize', () => {
    const runtime = renderSelectionResize({
      lockedAspectRatio: -2,
      nodes: [createNode('node-1', { x: 10, y: 20 }, 100, 50, 'embed')],
    })
    ...
    const zone = screen.getByTestId('canvas-selection-resize-zone-right')
    act(() => {
      fireEvent.pointerDown(zone, { button: 0, pointerId: 1, clientX: 110, clientY: 45 })
      fireEvent.pointerMove(window, { pointerId: 1, clientX: 160, clientY: 45 })
    })

    expectMapEntries(runtime.nodeActions.onResizeMany, [
      ['node-1', { width: 150, height: 50, position: { x: 10, y: 20 } }],
    ])
  })
  ...
  it('keeps changed modifier keys active for later resize pointer moves', () => {
    const runtime = renderSelectionResize()
    ...
    act(() => {
      fireEvent.keyDown(window, { key: 'Shift' })
      fireEvent.pointerMove(window, { pointerId: 1, clientX: 130, clientY: 65 })
    })
  ...
  it('removes the listener set that started the active resize after a rerender', () => {
