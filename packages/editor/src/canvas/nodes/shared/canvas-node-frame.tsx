...
export function CanvasNodeFrame({
  ...
  const { remoteNodeHighlights } = useCanvasCollaborationRuntime()
  const { visuallySelected, pendingPreviewActive, pendingSelected, selected } =
    useCanvasNodeVisualSelection(id)
  const hasMultipleVisualSelectedNodes = useCanvasEngineSelector(getHasMultipleVisualSelectedNodes)
  const highlight = interactiveRenderMode ? remoteNodeHighlights.get(id) : undefined
  const showSelectionChrome = interactiveRenderMode && Boolean(highlight)
  const showSelectionIndicator =
    interactiveRenderMode &&
    visuallySelected &&
    !highlight &&
    (pendingPreviewActive || dragging || hasMultipleVisualSelectedNodes)
...
function getHasMultipleVisualSelectedNodes(snapshot: CanvasEngineSnapshot) {
  const { pendingPreview } = snapshot.selection
  return pendingPreview.kind === 'active'
    ? pendingPreview.nodeIds.size > 1
    : snapshot.selection.nodeIds.size > 1
}
