...
export function useCanvasResizeSession(): CanvasSelectionResizeSession | null {
  ...
  const modifiersRef = useRef({
    primaryPressed: readPrimaryModifier(modifiers),
    shiftPressed: readShiftModifier(modifiers),
  })
  ...
  const updateResize = useCallback(
    (event: PointerEvent, commit: boolean) => {
      ...
      const square = modifiersRef.current.shiftPressed
      const snap = modifiersRef.current.primaryPressed
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      if (event.key === 'Shift') {
        modifiersRef.current = { ...modifiersRef.current, shiftPressed: true }
        updateResizeForSession(true, modifiersRef.current.primaryPressed)
      }

      if (event.key === 'Control' || event.key === 'Meta') {
        modifiersRef.current = { ...modifiersRef.current, primaryPressed: true }
        updateResizeForSession(modifiersRef.current.shiftPressed, true)
      }
    },
  ...
  const handleKeyUp = useCallback(
    (event: KeyboardEvent) => {
      if (event.key === 'Shift') {
        modifiersRef.current = { ...modifiersRef.current, shiftPressed: false }
        updateResizeForSession(false, modifiersRef.current.primaryPressed)
      }

      if (event.key === 'Control' || event.key === 'Meta') {
        modifiersRef.current = { ...modifiersRef.current, primaryPressed: false }
        updateResizeForSession(modifiersRef.current.shiftPressed, false)
      }
    },
    [updateResizeForSession],
  )
  ...
  const startSelectionResize = (
    ...
    activeResizeRef.current = {
      ...
      targetBounds: canvasEngineRef.current
        ...
        .nodes.filter(
          (candidate) =>
            // Stroke nodes use path geometry instead of rectangular resize snapping bounds.
  ...
  const resizeAxes = getSelectionResizeAxes(selection)
  const zones = getCanvasResizeHandleDescriptors(resizeAxes).map(
    ({ position, cursorClassName }) => ({
      position,
      cursorClassName,
      style: getResizeZoneStyle(position, resizeAxes),
...
function getSelectionNodeResizeSize({
  ...
  if (isValidLockedAspectRatio(metadata.lockedAspectRatio) && metadata.resizeAxes === 'both') {
...
function getSingleSelectionLockedAspectRatio(selection: {
  ...
  return isValidLockedAspectRatio(lockedAspectRatio) ? lockedAspectRatio : undefined
}
...
function getMinimumSelectionResizeSize(selection: {
  ...
  for (const { bounds, metadata } of selection.nodes) {
    if (bounds.width > 0 && metadata.resizeAxes !== 'vertical') {
      const lockedMinimumWidth = isValidLockedAspectRatio(metadata.lockedAspectRatio)
    ...
    if (bounds.height > 0 && metadata.resizeAxes !== 'horizontal') {
      const lockedMinimumHeight = isValidLockedAspectRatio(metadata.lockedAspectRatio)
...
function getResizeZoneStyle(
  ...
  const sideOffset = resizeAxes === 'both' ? halfCornerSize : 0
