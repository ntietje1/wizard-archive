...
export interface CanvasDomRuntime {
  ...
  subscribeViewportSurfaceBounds: ReturnType<
    typeof createCanvasDomRegistry
  >['subscribeViewportSurfaceBounds']
...
export function createCanvasDomRuntime(): CanvasDomRuntime {
  ...
  const scheduler = createCanvasRenderScheduler({
    ...
    afterFlush: () => pendingNodeDataPatches.clear(),
  })
  ...
  return {
    ...
    subscribeViewportSurfaceBounds: registry.subscribeViewportSurfaceBounds,
    ...
    scheduleNodeDataPatches: (
      ...
      for (const [nodeId, patch] of updates) {
        ...
        const nextPatch = {
          ...internalNode.node.data,
          ...pendingNodeDataPatches.get(nodeId),
          ...patch,
        }
        pendingNodeDataPatches.set(nodeId, nextPatch)
        mergedUpdates.set(nodeId, nextPatch)
      }
