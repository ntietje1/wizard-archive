...
describe('createCanvasDragController', () => {
  it('resolves the final release position before ending a drag session', () => {
    ...
    controller.begin(testCanvasNodeId('node-1'), createMouseEvent('mousedown', 0, 0))
    controller.update(createMouseEvent('mousemove', 10, 0))
    controller.commit(createMouseEvent('mouseup', 30, 0))

    expect(onEnd).toHaveBeenCalledTimes(1)
    expect(onEnd.mock.calls[0]?.[0].resolvedPositions.get(testCanvasNodeId('node-1'))).toEqual({
      x: 30,
      y: 0,
    })
  ...
  it('ends an active engine drag when destroyed', () => {
    ...
    controller.begin(testCanvasNodeId('node-1'), createMouseEvent('mousedown', 0, 0))
    controller.update(createMouseEvent('mousemove', 10, 0))
    expect(engine.getSnapshot().nodeLookup.get(testCanvasNodeId('node-1'))?.dragging).toBe(true)

    controller.destroy()

    expect(engine.getSnapshot().nodeLookup.get(testCanvasNodeId('node-1'))?.dragging).toBe(false)
  ...
  it('reverts active drag positions and reports the cancelled start state once', () => {
    ...
    expect(engine.getSnapshot().nodeLookup.get(testCanvasNodeId('node-1'))?.node.position).toEqual({
      x: 0,
      y: 0,
    })
    expect(engine.getSnapshot().nodeLookup.get(testCanvasNodeId('node-1'))?.dragging).toBe(false)
    ...
    expect(onCancel.mock.calls[0]?.[0].resolvedPositions).toEqual(
      new Map([[testCanvasNodeId('node-1'), { x: 0, y: 0 }]]),
    )
  ...
  it('does not let snapping move the locked axis during shift drag', () => {
    ...
    controller.begin(testCanvasNodeId('dragged'), createMouseEvent('mousedown', 0, 0))
    controller.update(createMouseEvent('mousemove', 10, 2))
    controller.commit(createMouseEvent('mouseup', 10, 2))

    expect(onEnd.mock.calls[0]?.[0].resolvedPositions.get(testCanvasNodeId('dragged'))).toEqual({
      x: 10,
      y: 0,
    })
