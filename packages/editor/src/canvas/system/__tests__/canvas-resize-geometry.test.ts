...
describe('resolveCanvasResize', () => {
  ...
  it('drops resize snap guides that minimum-size clamping prevents', () => {
    const result = resize({
      handlePosition: 'right',
      startBounds: { x: 0, y: 0, width: 100, height: 40 },
      currentPoint: { x: 108, y: 20 },
      minWidth: 120,
      targetBounds: [{ x: 109, y: 0, width: 1, height: 40 }],
      snap: true,
    })
  ...
  it('keeps locked-aspect corner snapping constrained to the opposite anchor', () => {
    const result = resize({
      handlePosition: 'bottom-right',
      startBounds: { x: 10, y: 20, width: 100, height: 50 },
      currentPoint: { x: 158, y: 100 },
      lockedAspectRatio: 2,
      targetBounds: [{ x: 160, y: 0, width: 20, height: 20 }],
      snap: true,
    })

    expect(result.bounds).toEqual({ x: 10, y: 20, width: 150, height: 75 })
    expect(result.guides).toContainEqual({
      orientation: 'vertical',
      position: 160,
      start: 0,
  ...
  it('keeps square corner snapping constrained to the opposite anchor', () => {
    const result = resize({
      handlePosition: 'bottom-right',
      startBounds: { x: 10, y: 20, width: 100, height: 50 },
      currentPoint: { x: 158, y: 200 },
      square: true,
      targetBounds: [{ x: 160, y: 0, width: 20, height: 20 }],
      snap: true,
    })

    expect(result.bounds).toEqual({ x: 10, y: 20, width: 150, height: 150 })
    expect(result.guides).toContainEqual({
      orientation: 'vertical',
      position: 160,
      start: 0,
...
describe('canvas resize handle helpers', () => {
  const bounds = { x: 10, y: 20, width: 100, height: 50 }

  it('resolves handle direction and opposite corner anchors', () => {
    expect(getHandleDirection('top-left')).toEqual({ x: -1, y: -1 })
    expect(getHandleDirection('bottom-right')).toEqual({ x: 1, y: 1 })
    expect(getHandleDirection('top')).toEqual({ x: 0, y: -1 })
    expect(getOppositeCorner(bounds, 'top-left')).toEqual({ x: 110, y: 70 })
    expect(getOppositeCorner(bounds, 'bottom-right')).toEqual({ x: 10, y: 20 })
  })

  it('resolves axis anchors and leading handles', () => {
    expect(getResizeAxisAnchorPoint(bounds, 'right')).toEqual({ x: 10, y: 45 })
    expect(getResizeAxisAnchorPoint(bounds, 'top')).toEqual({ x: 60, y: 70 })
    expect(isLeadingResizeHandle('left', 'x')).toBe(true)
    expect(isLeadingResizeHandle('right', 'x')).toBe(false)
    expect(isLeadingResizeHandle('top', 'y')).toBe(true)
    expect(isLeadingResizeHandle('bottom', 'y')).toBe(false)
  })
})
