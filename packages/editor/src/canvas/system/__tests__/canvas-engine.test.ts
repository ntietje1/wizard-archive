...
describe('createCanvasEngine', () => {
  ...
  it('skips projection and notification for unchanged document snapshots', () => {
    const engine = createCanvasEngine()
    const nodes = [createNode('a', 0)]
    const edges = [createEdge('edge-1', 'a', 'a')]
    engine.setDocumentSnapshot({ nodes, edges })
    const version = engine.getSnapshot().version
    const nodeLookup = engine.getSnapshot().nodeLookup
    const listener = vi.fn()
    engine.subscribe(listener)

    engine.setDocumentSnapshot({ nodes })
    engine.setDocumentSnapshot({ edges })
    engine.setDocumentSnapshot({})

    expect(listener).not.toHaveBeenCalled()
    expect(engine.getSnapshot().version).toBe(version)
    expect(engine.getSnapshot().nodeLookup).toBe(nodeLookup)
  })
  ...
  it('notifies subscribers when live selection previews change', () => {
  ...
  it('commits additive gesture previews against the gesture start selection', () => {
    const engine = createCanvasEngine()
    engine.setDocumentSnapshot({
      nodes: [createNode('a', 0), createNode('b', 1)],
    })
  ...
  it('notifies subscribers when live resize updates change node bounds', () => {
    const { domRuntime, engine } = createEngineWithDomRuntime()
    const source = { ...createNode('source', 0), width: 100, height: 50 }
    const target = {
      ...createNode('target', 1),
    ...
    expect(edgePath.getAttribute('d')).toBeTruthy()
    expect(engine.getSnapshot().nodes[0]).toEqual({
      ...source,
      width: 160,
      height: 80,
      position: { x: 20, y: 10 },
  ...
  it('merges visual node data patches with the current engine data before writing DOM styles', () => {
    ...
    expect(nodeSurface.style.borderColor).toBe('rgb(0, 255, 0)')
  ...
  it('applies document node layout patches to registered DOM nodes', () => {
    const { domRuntime, engine } = createEngineWithDomRuntime()
    const nodeElement = document.createElement('div')
    engine.setDocumentSnapshot({
  ...
  it('applies document edge style patches to registered edge paths', () => {
    const { domRuntime, engine } = createEngineWithDomRuntime()
    ...
    engine.setDocumentSnapshot({
      nodes: [createNode('source', 0), createNode('target', 1)],
      edges: [createEdge('edge-1', 'source', 'target')],
    })

    engine.patchEdges(new Map([['edge-1', { style: { stroke: '#00ff00', strokeWidth: 6 } }]]))
    domRuntime.flush()

    expect(path.style.stroke).toBe('rgb(0, 255, 0)')
    expect(path.dataset.canvasAuthoredStrokeWidth).toBe('6')
  ...
  it('uses measured node dimensions when document dimensions are absent', () => {
    ...
    expect(engine.getSnapshot().nodes[0]).not.toHaveProperty('width')
    expect(engine.getSnapshot().nodes[0]).not.toHaveProperty('height')
  ...
  it('recomputes connected edge paths when auto-sized node measurements change', () => {
    const { domRuntime, engine } = createEngineWithDomRuntime()
    const edgePath = document.createElementNS('http://www.w3.org/2000/svg', 'path')
    const unregister = domRuntime.registerEdgePaths('edge-1', {
      path: edgePath,
