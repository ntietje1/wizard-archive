...
describe('canvas engine managers', () => {
  ...
  it('preserves node projection references when only edges change', () => {
    const source = createNode('source', 0)
    ...
    const edgeOnlyProjection = projectCanvasDocumentSnapshot({
      snapshot: projected,
      edges: [createEdge('edge-2', 'target', 'source')],
      draggingNodeIds: EMPTY_SET,
    })

    expect(edgeOnlyProjection.nodes).toBe(projected.nodes)
    expect(edgeOnlyProjection.nodeIds).toBe(projected.nodeIds)
    expect(edgeOnlyProjection.nodeLookup).toBe(projected.nodeLookup)
    expect(edgeOnlyProjection.dirtyNodeIds).toBe(EMPTY_SET)
  })
  ...
  it('marks cleared selection preview ids dirty when direct selection replaces a gesture', () => {
    const selectionManager = createCanvasSelectionManager()
    const projected = projectCanvasDocumentSnapshot({
      snapshot: createSnapshot(),
      nodes: [createNode('a', 0), createNode('b', 1)],
  ...
  it('marks cleared selection preview ids dirty when a new gesture starts', () => {
    const selectionManager = createCanvasSelectionManager()
    const projected = projectCanvasDocumentSnapshot({
      snapshot: createSnapshot(),
      nodes: [createNode('a', 0), createNode('b', 1)],
