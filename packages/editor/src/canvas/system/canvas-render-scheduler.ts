...
import { getCanvasNodeSurfaceStyle } from '../node-surface-style'
...
import type { CanvasCameraState } from './canvas-engine-types'
import type { CanvasNodeSurfaceStyleData } from '../node-surface-style'
...
export function createCanvasRenderScheduler({
  ...
  const applyStrokeNodePaths = (
    ...
    const highlightPath = getCachedStrokeDetailPath(
      ...
      strokeData,
      resolveCanvasScreenMinimumStrokeWidth(
        getStrokeHighlightPathSize(strokeData.size, currentViewportZoom),
        currentViewportZoom,
      ),
    )
  ...
  const applyCullingDiff = () => {
    ...
    domRegistry.applyCullingDiff({
      nodeIds: new Map(pendingCulledNodeIds),
      edgeIds: new Map(pendingCulledEdgeIds),
    })
