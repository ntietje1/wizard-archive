...
type FreeformResizeSnapInput = Omit<
  ResizeSnapInput,
  'currentPoint' | 'lockedAspectRatio' | 'square'
>
...
export function resolveResizeSnap({
  ...
  lockedAspectRatio,
  square,
}: ResizeSnapInput): { bounds: CanvasResizeBounds; guides: ReadonlyArray<CanvasDragGuide> } | null {
...
function resolveFreeformResizeSnap({
  ...
  return {
    ...
    guides: [xSnap ?? null, ySnap ?? null].flatMap((snap) =>
      snap && isResizeSnapPointPreserved(snappedBounds, handlePosition, snap.axis, snap.point)
        ? [snap.guide]
        : [],
    ),
  }
...
function resolveConstrainedResizeSnap({
  ...
  return {
    bounds: snappedBounds,
    guides: isResizeSnapPointPreserved(
      snappedBounds,
      handlePosition,
      bestCandidate.axis,
      bestCandidate.point,
    )
      ? [createResizeAxisGuide(bestCandidate)]
      : [],
  }
}
...
function getBestResizeAxisSnap({
  ...
  targetBounds: ReadonlyArray<CanvasResizeBounds>
  threshold: number
}): { axis: ResizeAxis; point: number; guide: CanvasDragGuide } | null {
  const candidates = targetBounds.flatMap((targetBoundsItem) =>
    collectResizeAxisCandidates({
      axis,
      bounds,
...
function isResizeSnapPointPreserved(
  bounds: CanvasResizeBounds,
  handlePosition: CanvasResizeHandlePosition,
  axis: ResizeAxis,
  point: number,
) {
  const handlePoint = getResizeHandlePoint(bounds, handlePosition)
  return handlePoint[axis] === point
}
...
function collectResizeAxisCandidates({
  ...
  const metrics = getResizeAxisSnapMetrics({
    axis,
    bounds,
    targetBounds,
    handlePosition,
  ...
  return metrics.targetValues.flatMap((targetValue) => [
    createSnapCandidate({
      axis,
      bounds,
      targetBounds,
      draggedValue: metrics.draggedEdgeValue,
      targetValue,
      point: targetValue,
    }),
    createSnapCandidate({
      ...
      bounds,
      targetBounds,
      draggedValue: metrics.draggedCenterValue,
      targetValue,
      // Mirror the dragged center around the anchor so the resized center lands on this snap target.
      point: 2 * targetValue - metrics.anchorValue,
    }),
  ])
}

function getResizeAxisSnapMetrics({
  axis,
  bounds,
  targetBounds,
  handlePosition,
  ...
  if (axis === 'x') {
    return {
      ...
      draggedCenterValue: bounds.x + bounds.width / 2,
      draggedEdgeValue: isLeadingResizeHandle(handlePosition, axis)
        ? bounds.x
  ...
  return {
    ...
    draggedCenterValue: bounds.y + bounds.height / 2,
    draggedEdgeValue: isLeadingResizeHandle(handlePosition, axis)
      ? bounds.y
