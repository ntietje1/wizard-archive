...
export function createCanvasDragController({
  ...
  const commit: CanvasDragController['commit'] = (event) => {
    ...
    if (session.started) {
      updateSession(event, { emit: false })
      endSession(event)
    }
  ...
  const cancel: CanvasDragController['cancel'] = (event) => {
    ...
    if (session.started) {
      ...
      canvasEngine.stopDrag()
      callbacks.onCancel?.(createDragEvent(event, session, session.startPositions, [], false))
    }
  ...
  function startSession(sourceEvent: PointerEvent | MouseEvent) {
    ...
    callbacks.onStart?.(createDragEvent(sourceEvent, session, session.startPositions, [], false))
  }

  function updateSession(sourceEvent: PointerEvent | MouseEvent, options: { emit?: boolean } = {}) {
    if (!session) {
      return
    }
    ...
    if (options.emit !== false) {
      callbacks.onDrag?.(
        createDragEvent(sourceEvent, session, resolved.positions, resolved.guides, false),
      )
    }
  }
  ...
  function endSession(sourceEvent: PointerEvent | MouseEvent) {
    ...
    canvasEngine.stopDrag()
    callbacks.onEnd?.(
      createDragEvent(sourceEvent, session, session.lastResolvedPositions, [], true),
    )
  }
...
function resolveDragPositions({
  ...
  return {
    guides,
    positions: filterMissingDragPositionsInPlace(resolvedPositions, session.existingNodeIds),
  }
}
...
function createDragEvent(
  ...
  const firstPosition = resolvedPositions.get(session.activeNodeId)
