...
export function projectCanvasDocumentSnapshot({
  ...
  const nodeLookup = nodes
    ...
    : snapshot.nodeLookup
  const edgeLookup = edges ? createEdgeLookup(nextEdges, selection.edgeIds) : snapshot.edgeLookup
  const edgeIdsByNodeId = edges ? createEdgeAdjacency(nextEdges) : snapshot.edgeIdsByNodeId
...
function retainExistingIds(selectedIds: ReadonlySet<string>, existingIds: ReadonlyArray<string>) {
  const existing = new Set(existingIds)
  return new Set([...selectedIds].filter((id) => existing.has(id)))
}

function preserveDraggingNodeLookup(
  nextLookup: ReadonlyMap<string, CanvasInternalNode>,
  previousLookup: ReadonlyMap<string, CanvasInternalNode>,
  draggingNodeIds: ReadonlySet<string>,
  ...
  let mergedLookup: Map<string, CanvasInternalNode> | null = null
  for (const nodeId of draggingNodeIds) {
    const previousNode = previousLookup.get(nodeId)
    if (!previousNode || !nextLookup.has(nodeId)) {
      continue
    }

    mergedLookup ??= new Map(nextLookup)
    mergedLookup.set(nodeId, {
      ...previousNode,
      dragging: true,
    })
  }

  return mergedLookup ?? nextLookup
}
