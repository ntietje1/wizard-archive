...
export interface CanvasDomRegistry {
  ...
  getStrokeNodePathEntries: () => Iterable<readonly [string, CanvasRegisteredStrokeNodePaths]>
...
export function createCanvasDomRegistry(): CanvasDomRegistry {
  ...
  return {
    ...
    getStrokeNodePathEntries: () => strokeNodePaths.entries(),
...
function applyCullingDisplay(element: Element, isCulled: boolean) {
  ...
  if (isCulled) {
    if (!htmlElement.hasAttribute('data-canvas-culled')) {
      htmlElement.dataset.canvasPreviousDisplay = htmlElement.style.display
    }
    htmlElement.style.display = 'none'
    htmlElement.setAttribute('data-canvas-culled', 'true')
  } else {
    if (htmlElement.hasAttribute('data-canvas-culled')) {
      htmlElement.style.display = htmlElement.dataset.canvasPreviousDisplay ?? ''
      delete htmlElement.dataset.canvasPreviousDisplay
    }
