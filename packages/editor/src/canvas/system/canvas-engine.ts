...
export function createCanvasEngine(config: { domRuntime?: CanvasDomRuntime } = {}): CanvasEngine {
  ...
  const setRuntimeSnapshot = (
    ...
    store.setSnapshot(next, { incrementVersion: options.incrementVersion, notify: options.notify })
    reconcileCulling()
  }

  const setDocumentSnapshot: CanvasEngine['setDocumentSnapshot'] = ({ nodes, edges }) => {
    const snapshot = store.getSnapshot()
    const hasNodeUpdate = nodes !== undefined && nodes !== snapshot.nodes
    const hasEdgeUpdate = edges !== undefined && edges !== snapshot.edges
    if (!hasNodeUpdate && !hasEdgeUpdate) {
      return
    }

    clearRemovedStrokePathCache(snapshot.nodes, hasNodeUpdate ? nodes : undefined)
    commit(
      projectCanvasDocumentSnapshot({
        snapshot,
        nodes: hasNodeUpdate ? nodes : undefined,
        edges: hasEdgeUpdate ? edges : undefined,
        draggingNodeIds,
      }),
    )
  }
  ...
  const applySelectionPreviewUpdate = (next: Omit<CanvasEngineSnapshot, 'version'> | null) => {
    if (next) {
      setRuntimeSnapshot(next, { incrementVersion: true, notify: true })
    }
  }
  ...
  const updateResize: CanvasEngine['updateResize'] = (updates) => {
    ...
    setRuntimeSnapshot(update.snapshot, { incrementVersion: true, notify: true })
  ...
  return {
    ...
    setSelectionGesturePreview: (selection) =>
      applySelectionPreviewUpdate(
        selectionManager.setGesturePreview(store.getSnapshot(), selection),
      ),
