...
function buildVerticalRunStepPoints({
  end,
  sourceStub,
  start,
  targetStub,
...
function buildRelaxedHorizontalStepPoints(props: StepGeometryProps): Array<Point2D> {
...
function buildRelaxedVerticalStepPoints(props: StepGeometryProps): Array<Point2D> {
  const start = { x: props.sourceX, y: props.sourceY }
  const end = { x: props.targetX, y: props.targetY }
...
function buildHorizontalHandlesStepPoints(
...
function buildVerticalHandlesStepPoints(
  props: StepGeometryProps,
  splitCoordinates: StepSplitCoordinates,
  start: Point2D,
  end: Point2D,
  ...
  if (
    areVerticalHandlesFacing(props.sourcePosition, props.targetPosition, sourceStub.y, targetStub.y)
  ) {
    return buildHorizontalRunStepPoints({
      start,
      end,
...
function buildStepPoints(
  ...
  if (sourceHorizontal && targetHorizontal) {
    return buildHorizontalHandlesStepPoints(
      props,
      splitCoordinates,
      start,
      end,
  ...
  if (!sourceHorizontal && !targetHorizontal) {
    return buildVerticalHandlesStepPoints(
      props,
      splitCoordinates,
      start,
      end,
...
export function buildStepCanvasEdgeGeometryFromEdge(
  ...
  return buildCanvasEdgeGeometryFromResolvedEndpoints(edge, nodesById, (endpoints, nodes) => {
