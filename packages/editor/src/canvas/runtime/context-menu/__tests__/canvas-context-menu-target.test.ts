...
describe('resolveCanvasContextMenuTarget', () => {
  ...
  it('returns an embed-node target for a single valid embed selection', () => {
    ...
    expect(resolved).toEqual({
      ...
      nodeId: 'embed-1',
      nodeType: 'embed',
      target: { kind: 'resource', resourceId: 'note-1' },
    })
  })
  ...
  it('returns a pane target when selected ids no longer resolve', () => {
    expect(
      resolveCanvasContextMenuTarget(
        selectionSnapshot({
          nodeIds: new Set(['missing-node']),
          edgeIds: new Set(['missing-edge']),
        }),
  ...
  it('uses the resolved node and edge entities to choose the target kind', () => {
    const snapshot = createContextMenuSnapshot({
      nodes: [
        {
          id: 'text-1',
    ...
    expect(
      resolveCanvasContextMenuTarget(
        selectionSnapshot({ nodeIds: new Set(['text-1']), edgeIds: new Set(['missing-edge']) }),
        snapshot,
      ),
    ).toEqual({
      kind: 'node-selection',
    ...
    expect(
      resolveCanvasContextMenuTarget(
        selectionSnapshot({ nodeIds: new Set(['missing-node']), edgeIds: new Set(['edge-1']) }),
        snapshot,
      ),
    ).toEqual({
      kind: 'edge-selection',
