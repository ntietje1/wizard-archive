...
describe('useCanvasSelectionController', () => {
  ...
  it('sets and clears engine-owned selection snapshots', () => {
    ...
    expect(setLocalSelection).toHaveBeenCalledWith({
      nodeIds: new Set(['a', 'b']),
      edgeIds: new Set(['edge-1']),
    })
  ...
  it('publishes edge-only selection instead of clearing remote presence', () => {
    const { result, setLocalSelection } = renderController()

    act(() => {
      result.current.toggleEdge('edge-1', false)
    })
