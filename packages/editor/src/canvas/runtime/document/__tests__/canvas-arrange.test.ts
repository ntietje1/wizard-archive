...
vi.mock('../../../nodes/shared/canvas-node-bounds', () => ({
...
describe('createCanvasArrangePlan', () => {
  ...
  it('computes aggregate bounds without spreading the full selected node list', () => {
    const nodes = Array.from({ length: 32 }, (_, index) =>
      createNode(`node-${index}`, index * 10, index * 5, 4, 6),
    )
    const { nodesMap } = createNodesMap(nodes)
    const selection = createSelection(nodes.map(({ id }) => id))
    const minSpy = vi.spyOn(Math, 'min').mockImplementation((...values: Array<number>) => {
      if (values.length > 2) {
        throw new RangeError('too many arguments')
      }
      return values.reduce((smallest, value) => (value < smallest ? value : smallest), Infinity)
    })
    const maxSpy = vi.spyOn(Math, 'max').mockImplementation((...values: Array<number>) => {
      if (values.length > 2) {
        throw new RangeError('too many arguments')
      }
  ...
  it('preserves node position offsets when flipping bounds', () => {
    const { nodesMap } = createNodesMap([
      createOffsetNode('a', 5, 23, 0, 20, 20, 10),
      createOffsetNode('b', 47, 4, 50, 0, 10, 20),
      createOffsetNode('c', 110, 75, 100, 80, 40, 30),
