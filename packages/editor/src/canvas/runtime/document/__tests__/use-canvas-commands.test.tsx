...
describe('useCanvasDocumentCommands', () => {
  ...
  it('preserves selection, clipboard, and canvas maps when cut deletion fails', () => {
    ...
    useCanvasClipboardStore.getState().setClipboard(previousClipboard)
  ...
  it('limits edge-only selections to edge-capable commands', () => {
    const { doc, nodesMap, edgesMap } = createCanvasMaps()
    ...
    expect(result.current.delete.canRun()).toBe(true)
    expect(result.current.reorder.canRun({ direction: 'bringToFront' })).toBe(true)
    expect(result.current.copy.canRun()).toBe(false)
    expect(result.current.cut.canRun()).toBe(false)
    expect(result.current.duplicate.canRun()).toBe(false)
