...
export function createCanvasReorderPlan(
  ...
  const orderedElementIds = currentElements.map((element) => element.id)
  const existingSelectedIds = new Set(orderedElementIds.filter((id) => selectedIds.has(id)))
  if (existingSelectedIds.size === 0) {
    return null
  }
  const reorderedIds = reorderCanvasElementIds(orderedElementIds, existingSelectedIds, direction)
