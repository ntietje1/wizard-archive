...
describe('useCanvasViewportInteractions', () => {
  it('routes wheel, middle-button pan, and hand-tool primary pan to the engine viewport', () => {
    ...
    expect(viewportController.handlePanPointerDown).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ button: 1, buttons: 4 }),
    )
    expect(viewportController.handlePanPointerDown).toHaveBeenNthCalledWith(
      2,
  ...
  it('captures pan pointerdown before descendants can stop propagation', () => {
    const viewportController = createViewportControllerMock()

    render(<Harness canPrimaryPan={() => true} viewportController={viewportController} />)

    const scene = document.querySelector('.canvas-scene') as HTMLElement
    scene.addEventListener('pointerdown', (event) => event.stopPropagation())
    scene.dispatchEvent(createPointerEvent('pointerdown', { button: 0 }))

    expect(viewportController.handlePanPointerDown).toHaveBeenCalledTimes(1)
  })

  it('creates mouse pointer events with browser buttons bitmask values', () => {
    expect(createPointerEvent('pointerdown', { button: 0 }).buttons).toBe(1)
    expect(createPointerEvent('pointerdown', { button: 1 }).buttons).toBe(4)
    expect(createPointerEvent('pointerdown', { button: 2 }).buttons).toBe(2)
  })
})
...
function createPointerEvent(type: string, init: { button: number }): PointerEvent {
  if (typeof PointerEvent !== 'undefined') {
    return new PointerEvent(type, {
      bubbles: true,
      button: init.button,
      buttons: getMouseButtonsBitmask(init.button),
      cancelable: true,
      isPrimary: true,
      pointerId: 0,
      pointerType: 'mouse',
...
function getMouseButtonsBitmask(button: number) {
  if (button === 1) {
    return 4
  }
  if (button === 2) {
