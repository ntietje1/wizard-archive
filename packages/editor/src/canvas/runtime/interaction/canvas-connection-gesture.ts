...
export function useCanvasConnectionGesture({
  ...
  const handlePointerMove = useCallback(
    ...
    [canEdit, cancel, canvasEngine, paneRef],
  )

  const handlePointerUp = useCallback(
    (event: PointerEvent) => {
      if (!isActiveConnectionDraftPointer(event, connectionDraftRef)) {
        return
      }

      removeConnectionCompletionListeners(pointerUpRef, pointerCancelRef)
      if (!canEdit) {
        cancel()
        return
      }
...
function isActiveConnectionDraftPointer(
  event: PointerEvent,
  connectionDraftRef: RefObject<CanvasConnectionDraft | null>,
): boolean {
  const activeDraft = connectionDraftRef.current
  return Boolean(activeDraft && event.pointerId === activeDraft.pointerId)
}

function removeConnectionCompletionListeners(
  pointerUpRef: RefObject<(event: PointerEvent) => void>,
  pointerCancelRef: RefObject<(event: PointerEvent) => void>,
) {
