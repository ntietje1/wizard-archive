...
export function useCanvasNodeDragHandlers({
  ...
  controllerRef.current ??= createCanvasDragController({
    ...
    getCanStartDrag: () =>
      optionsRef.current.nodesDoc !== null &&
      optionsRef.current.nodesDoc !== undefined &&
      optionsRef.current.getCanStartDrag(),
    callbacks: {
      onStart: (event) =>
...
function handleDragEnd(
  ...
  try {
    ...
    canvasDevLogger.error('useCanvasNodeDragHandlers: failed to persist node drag positions', error)
  } finally {
    const localDraggingIds = localDraggingIdsRef.current
    if (localDraggingIds) {
      for (const nodeId of event.draggedNodeIds) {
