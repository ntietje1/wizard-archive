...
import { restoreBlockNoteSelection } from '../../rich-text/blocknote/blocknote-selection-adapter'
...
function materializeBlockTextColor<TBlock extends CanvasTextBlockWithChildren>(
  ...
  if (nextContent === block.content && nextChildren === block.children) return block
  return {
    ...block,
    content: nextContent,
...
function materializeInlineTextItemColor(item: unknown, textColor: string): unknown {
  ...
  if (typeof inlineItem.text !== 'string' || inlineItem.text.length === 0) return item
