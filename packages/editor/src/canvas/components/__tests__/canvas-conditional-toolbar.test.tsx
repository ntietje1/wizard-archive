...
type CanvasTextFormattingEditor = Parameters<
  typeof registerCanvasTextFormattingSession
>[0]['editor']
...
vi.mock('@wizard-archive/ui/shadcn/components/slider', () => ({
  Slider: ({
    value,
    min,
    max,
...
function createNode(
  ...
  return {
    ...
    data: {
      ...
      ...(options.sidebarItemId !== undefined
        ? {
            target: {
              kind: 'resource',
...
describe('CanvasConditionalToolbar', () => {
  ...
  it('shows edge tool defaults when nothing is selected and updates the tool edge type', () => {
    ...
    expect(canvasToolStore.getState().edgeType).toBe('step')
  })
  ...
  it('updates tool property state after changing tool color and stroke size', () => {
    ...
    expect(canvasToolStore.getState().strokeSize).toBe(8)
  ...
  it('refreshes draw tool defaults immediately when the tool store changes externally', () => {
    ...
    act(() => {
      ...
      canvasToolStore.getState().setStrokeSize(12)
    })
  ...
  it('applies text color to the active rich-text selection instead of the node default', () => {
    ...
    }) as unknown as CanvasTextFormattingEditor
  ...
  it('prevents custom color picker pointer down from taking focus from the active editor', () => {
    renderToolbar()
    emitSelection([createNode('text', { textColor: 'var(--t-blue)' })])
    ...
    const colorPickerButton = within(textGroup!).getByRole('button', {
      name: 'Open color picker',
    })
    const event = new MouseEvent('pointerdown', { bubbles: true, cancelable: true })

    colorPickerButton.dispatchEvent(event)
  ...
  it('does not select a color preset for an opacity-zero custom color', () => {
    ...
    act(() => {
      canvasToolStore.getState().setStrokeColor('var(--t-red)')
      canvasToolStore.getState().setStrokeOpacity(0)
    })
    emitSelection([])
  ...
  it('commits one-digit and two-digit stroke sizes from the numeric input', () => {
    ...
    expect(canvasToolStore.getState().strokeSize).toBe(9)
    expect(getStrokeSizeInput()).toHaveValue('9')

    input = getStrokeSizeInput()
    fireEvent.change(input, { target: { value: '99' } })
    fireEvent.blur(input)
    expect(canvasToolStore.getState().strokeSize).toBe(99)
    expect(getStrokeSizeInput()).toHaveValue('99')
  })

  it('commits fractional selected edge stroke sizes from the numeric input', () => {
    const patchEdge = vi.fn()
    renderToolbar({ patchEdge })
    const edge = createEdge({ stroke: 'var(--foreground)', strokeWidth: 1 })
    edgesMock.edges = [edge]
    emitSelectionState(selectionSnapshot(new Set<string>(), new Set([edge.id])))
  ...
  it('rejects invalid numeric input drafts and restores the current stroke size', () => {
    ...
    fireEvent.blur(input)
    expect(canvasToolStore.getState().strokeSize).toBe(4)
