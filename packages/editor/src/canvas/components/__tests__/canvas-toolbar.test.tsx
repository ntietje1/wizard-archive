...
describe('CanvasToolbar', () => {
  ...
  function createToolbarRuntime(canEdit = true) {
    const viewportController: CanvasViewportController = {
      getViewport: vi.fn(() => ({ x: 0, y: 0, zoom: 1 })),
      getZoom: vi.fn(() => 1),
      screenToCanvasPosition: vi.fn((position) => position),
    ...
    return createCanvasRuntime({ canEdit, history, viewportController })
  }
  ...
  it('keeps active tool state scoped to each canvas runtime', () => {
    const firstRuntime = createToolbarRuntime()
    const secondRuntime = createToolbarRuntime()

    render(
      <div>
        <section aria-label="First canvas">
          <CanvasRuntimeProvider {...firstRuntime}>
            <CanvasToolbar canEdit />
          </CanvasRuntimeProvider>
        </section>
        <section aria-label="Second canvas">
          <CanvasRuntimeProvider {...secondRuntime}>
            <CanvasToolbar canEdit />
          </CanvasRuntimeProvider>
        </section>
      </div>,
    ...
    const firstCanvas = screen.getByRole('region', { name: 'First canvas' })
    const secondCanvas = screen.getByRole('region', { name: 'Second canvas' })

    fireEvent.click(within(firstCanvas).getByRole('button', { name: 'Draw' }))

    expect(within(firstCanvas).getByRole('button', { name: 'Draw' })).toHaveAttribute(
      'aria-pressed',
      'true',
    )
    expect(within(secondCanvas).getByRole('button', { name: 'Draw' })).toHaveAttribute(
      'aria-pressed',
      'false',
    )
    expect(within(secondCanvas).getByRole('button', { name: 'Pointer' })).toHaveAttribute(
      'aria-pressed',
      'true',
    )
  })
