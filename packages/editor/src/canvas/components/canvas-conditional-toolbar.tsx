  ...
  return (
    <>
      {paintProperties.map((paintProperty) => {
        ...
        return (
          <div
            key={paintProperty.definition.id}
            className="flex flex-col gap-1"
            role="group"
            aria-label={paintProperty.definition.label}
          >
      ...
      {strokeSizeProperty && (
        <div
          className="flex flex-col gap-1"
          role="group"
          aria-label={strokeSizeProperty.definition.label}
        >
          <p className="text-[11px] font-medium text-muted-foreground">
      ...
      <div className="flex size-6 shrink-0 items-center justify-center rounded-sm border border-border focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-1">
        <input
          ...
          onChange={(event) => {
            dispatchDraftState({
              type: 'setDraftValue',
              value: sanitizeStrokeSizeDraft(event.currentTarget.value),
            })
          }}
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
...
function sanitizeStrokeSizeDraft(value: string) {
