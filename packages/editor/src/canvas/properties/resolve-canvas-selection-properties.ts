...
import { withActiveCanvasTextColorBinding } from './canvas-text-property-binding'
...
export function resolveCanvasSelectionProperties({
  ...
  activeFormattingSnapshot: CanvasTextFormattingSnapshot | null
    ...
    const selectedProperties = [
      ...selectedNodes.map<CanvasInspectableProperties>((node) =>
        getSelectedNodeInspectableProperties(
          node,
          patchNodeData,
          node.type === 'text' || isNoteEmbed(node),
          activeFormattingSnapshot,
        ),
      ),
...
function getSelectedNodeInspectableProperties(
  ...
  return withActiveCanvasTextColorBinding(properties, activeFormattingSnapshot)
}
