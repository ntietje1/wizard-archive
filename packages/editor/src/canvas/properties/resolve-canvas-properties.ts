...
function assertNever(value: never): never {
  throw new Error(`Unhandled canvas property binding kind: ${JSON.stringify(value)}`)
}
...
function createStrokeSizeProperty(
...
): CanvasStrokeSizeResolvedProperty | null {
  ...
  if (computedMin > computedMax) {
    // No single value can satisfy every selected binding; hide the shared setter.
    return null
  }
...
export function resolveCanvasProperties(
  ...
  return firstBindings.flatMap((binding) => {
    ...
    const resolvedProperty = createResolvedProperty(binding, resolvedMatches)
