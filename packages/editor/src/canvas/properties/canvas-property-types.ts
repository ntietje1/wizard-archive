...
export interface CanvasInspectableProperties {
  bindings: ReadonlyArray<CanvasPropertyBinding>
}

export const EMPTY_CANVAS_INSPECTABLE_PROPERTIES: CanvasInspectableProperties = Object.freeze({
  ...
  return {
    ...
    getValue: () => {
      ...
      return color === null
        ...
        : {
            color,
            opacity: binding.getOpacity?.() ?? definition.defaultValue.opacity,
          }
    },
    setValue: (value) => {
      if (binding.setValue) {
      ...
      if (!binding.setOpacity && value.opacity !== definition.defaultValue.opacity) {
        throw new Error(
          `Canvas paint property "${definition.id}" cannot apply opacity without setOpacity or a custom setValue`,
        )
      }
