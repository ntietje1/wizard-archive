...
describe('resolveCanvasProperties', () => {
  ...
  it('updates color-only paint bindings when the opacity stays at the property default', () => {
    const setColor = vi.fn()
    const properties = resolveCanvasProperties([
      {
        bindings: [
          bindCanvasPaintProperty(textColorCanvasProperty, {
            getColor: () => 'var(--t-blue)',
            setColor,
          }),
        ],
    ...
    expect(properties).toHaveLength(1)
    assert(properties[0]?.definition.kind === 'paint')
    const paintProperty = properties[0] as CanvasPaintResolvedProperty

    paintProperty.setValue({ color: 'var(--t-red)', opacity: 100 })

    expect(setColor).toHaveBeenCalledWith('var(--t-red)')
  })

  it('requires paint bindings to own opacity writes before exposing non-default opacity', () => {
    const setColor = vi.fn()
    const properties = resolveCanvasProperties([
      {
        bindings: [
    ...
    expect(() => paintProperty.setValue({ color: 'var(--t-red)', opacity: 50 })).toThrow(
      'cannot apply opacity',
    )
  })
  ...
  it('does not expose a shared stroke-size setter for disjoint binding ranges', () => {
    const setFillColor = vi.fn()
    const narrowStrokeSizeProperty = {
      ...testStrokeSizeProperty,
      min: 1,
      max: 3,
    } satisfies CanvasStrokeSizePropertyDefinition
    const wideStrokeSizeProperty = {
      ...testStrokeSizeProperty,
      min: 6,
      max: 9,
    } satisfies CanvasStrokeSizePropertyDefinition
    ...
    expect(properties).toHaveLength(1)
    assert(properties[0]?.definition.kind === 'paint')
    expect(properties[0].definition.id).toBe(testPaintProperty.id)
    expect(properties[0].value).toEqual({
      kind: 'value',
      value: { color: '#ff0000', opacity: 100 },
    })
  })
})
...
function createFormattingSnapshot({
  ...
  } as unknown as CanvasTextFormattingSnapshot
}
