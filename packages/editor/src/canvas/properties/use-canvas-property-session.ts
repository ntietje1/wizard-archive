...
export function useCanvasPropertySession({
  ...
  const revertPropertyPatches = (patches: CanvasPropertyPatchSet) => {
    const snapshot = canvasEngine.getSnapshot()
    const nodeDataPatches = new Map<string, CanvasNodeDataPatch>()
    for (const nodeId of patches.nodeDataPatches.keys()) {
      const node = snapshot.nodeLookup.get(nodeId)?.node
      if (node) nodeDataPatches.set(nodeId, node.data)
    }
    const edgePatches = new Map<string, CanvasEdgePatch>()
    for (const edgeId of patches.edgePatches.keys()) {
      const edge = snapshot.edgeLookup.get(edgeId)?.edge
      if (edge) edgePatches.set(edgeId, { type: edge.type, style: edge.style })
    }
    domRuntime.scheduleNodeDataPatches(snapshot, nodeDataPatches)
    domRuntime.scheduleEdgePatches(edgePatches)
  }
  ...
  const runPropertyPreviewChange = (applyChange: () => void) => {
    propertySessionController.startPropertySession({
      ...
      revertPatches: revertPropertyPatches,
