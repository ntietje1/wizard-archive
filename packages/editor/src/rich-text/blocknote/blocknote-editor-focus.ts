...
type BlockNoteEditorFocusPoint = { x: number; y: number }

export function focusEditorViewAtNearestPoint(view: EditorView, point: BlockNoteEditorFocusPoint) {
...
function focusEditorView(view: EditorView) {
  try {
    ...
    // Selection placement already succeeded; focus failures should not trigger fallback selection.
  }
}
