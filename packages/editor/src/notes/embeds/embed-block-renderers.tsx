...
function mapEmbedRenderProps(props: EmbedBlockRenderProps): NoteEmbedBlockBaseProps {
  ...
  return {
    ...
    editor: {
      ...
      extendTextSelectionToBlockBoundary: (blockId) => {
        editor.transact((transaction) => {
          const target = getNodeById(blockId, transaction.doc)
          if (!target) return

          const { anchor } = transaction.selection
          const targetStart = target.posBeforeNode
          const targetEnd = targetStart + target.node.nodeSize
          const head = anchor <= targetStart ? targetEnd : targetStart
...
function getExternalEmbedDataAttributes(props: NoteEmbedBlockProps) {
  return {
    ...
    'data-resource-id': props.targetKind === 'resource' ? props.resourceId : undefined,
