...
import { getWikiLinkContext, splitWikiLinkTargetAndDisplayName } from './utils'
import {
...
import type { CustomBlockNoteEditor, CustomValuePartialInlineContent } from '../editor-schema'
...
import { createUuidV4 } from '../value-block/value-transfer-plugin'
...
function useClosedMenuEnterShortcut({
  ...
  const forceOpenRef = useLatestRef(forceOpen)
  ...
  }, [editor, editorEl, forceOpenRef, menuShowing])
}
...
function useAutocompleteKeyboard({
  ...
  useEffect(() => {
    if (!editorEl || !menuShowing) return

    const onKeyDown = (event: KeyboardEvent) => {
      const state = keyboardStateRef.current
      if (!state.model) return

      const len = state.model.suggestions.length || 1
      switch (event.key) {
        case 'ArrowDown':
          event.preventDefault()
        ...
        case 'Enter':
          ...
          insertSelectedSuggestion(
            state.model,
            state.selectedIndex,
            state.insertFileLink,
            state.insertHeadingLink,
            state.insertValueInline,
          )
          break
        case 'Tab':
          event.preventDefault()
          continueSelectedSuggestion(
            ...
            state.insertFileLink,
            state.insertValueInline,
            state.continueFileLink,
            state.continueHeadingLink,
            state.continueFolderPath,
            state.closeMenu,
          )
          break
        case 'Escape':
    ...
    editorEl.addEventListener('keydown', onKeyDown, true)
    return () => editorEl.removeEventListener('keydown', onKeyDown, true)
  }, [editorEl, keyboardStateRef, menuShowing, setSelectedIndex])
}
...
function continueSelectedSuggestion(
  ...
  if (suggestion.item.type === RESOURCE_TYPES.notes) {
    continueFileLink(suggestion, model.context)
  } else if (suggestion.item.type === RESOURCE_TYPES.folders) {
...
function AutocompleteMenu({
  ...
  return createPortal(
    ...
    >
      <WikiLinkAutocompletePanel
        headingsPending={headingsPending}
        insertFileLink={insertFileLink}
        insertHeadingLink={insertHeadingLink}
        insertValueInline={insertValueInline}
        model={model}
...
function WikiLinkAutocompletePanel({
  headingsPending,
  insertFileLink,
  insertHeadingLink,
  insertValueInline,
