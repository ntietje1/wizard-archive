...
import { rewriteSameNoteValueReferences } from '../values/authoring'
import { NOTE_VALUE_DEFAULT_SLUG, NOTE_VALUE_SLUG_OPTIONS } from '../values/constants'
...
function registerValueTransferPlugin(
  ...
  let registeredEditor: TiptapEditorLike | null = null
  ...
  return () => {
    ...
    registeredEditor?.unregisterPlugin?.(valueTransferPluginKey)
    registeredEditor = null
  }
}

function replaceValueTransferPlugin(newPlugin: Plugin, plugins: Array<Plugin>): Array<Plugin> {
  return [...plugins.filter((plugin) => plugin.spec.key !== valueTransferPluginKey), newPlugin]
}
