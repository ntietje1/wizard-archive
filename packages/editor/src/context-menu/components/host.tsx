...
const RICH_SUBMENU_PORTAL_SELECTOR =
  '[data-context-menu-rich-submenu-portal], [data-slot="select-content"]'
...
function belongsToSubmenuInteraction(container: HTMLElement, target: EventTarget | null) {
  if (!(target instanceof Element)) return false
  return container.contains(target) || Boolean(target.closest(RICH_SUBMENU_PORTAL_SELECTOR))
}
...
function ContextMenuRichSubmenuItem({
  ...
  const closePanelAfterNestedPopupCheck = (container: HTMLElement, target: EventTarget | null) => {
    ...
    window.requestAnimationFrame(() => {
      if (document.querySelector(RICH_SUBMENU_PORTAL_SELECTOR)) return
  ...
  return (
    ...
    >
      <div ref={triggerRef} data-slot="context-menu-sub-trigger">
        <ContextMenuItem
          aria-haspopup="menu"
          aria-expanded={open}
          ...
          className={cn(
            'w-full data-open:bg-accent data-open:text-accent-foreground',
  ...
  if (menuItem.children && menuItem.children.length > 0) {
    ...
    return (
      ...
      >
        ...
        <ContextMenuSubContent data-item-surface-floating-command="true">
...
export function ContextMenuHost({
  ...
  const pendingOpenPositionRef = useRef<{ x: number; y: number } | null>(null)
  ...
  React.useEffect(() => {
    if (menu.isEmpty || !pendingOpenPositionRef.current) return

    const position = pendingOpenPositionRef.current
    pendingOpenPositionRef.current = null
    menuRef.current?.openAt(position)
  }, [menu.isEmpty])
