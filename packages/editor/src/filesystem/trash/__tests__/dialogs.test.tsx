...
describe('FileSystemPermanentDeleteDialog', () => {
  it('describes descendant deletion when permanently deleting selected folders', () => {
    const folder = createFolder({ id: testId('folder_with_child'), name: 'Scenes' })
    const child = createNote({ parentId: folder.id, name: 'Hidden Room' })
    const note = createNote({ id: testId('loose_note'), name: 'Loose Note' })
  ...
  it.each([
    {
      name: 'a leaf item',
      items: [createNote({ id: testId('trashed_note'), name: 'Loose Note' })],
    },
    {
      name: 'selected leaf items',
      items: [
        createNote({ id: testId('first_trashed_note'), name: 'First Note' }),
        createNote({ id: testId('second_trashed_note'), name: 'Second Note' }),
      ],
    },
  ])(
  ...
  it('blocks permanent deletion for folders when trash contents fail to load', async () => {
    const user = userEvent.setup()
    const folder = createFolder({ id: testId('trashed_folder'), name: 'Scenes' })
    const onConfirm = vi.fn()
