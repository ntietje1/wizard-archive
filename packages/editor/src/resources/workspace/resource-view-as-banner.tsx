...
export function ResourceViewAsBanner({ viewAs }: { viewAs: EditorRuntime['viewAs'] }) {
  if (viewAs.status !== 'available') return null
  const participant = viewAs.value.participants.find(
    (candidate) => candidate.id === viewAs.value.selectedParticipantId,
  )
    ...
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 z-50 overflow-hidden">
        <Banner
          ...
          border="top"
          actions={
            <BannerButton onClick={() => viewAs.value.select(null)}>
