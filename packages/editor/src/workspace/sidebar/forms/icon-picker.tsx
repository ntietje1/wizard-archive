import { useState } from 'react'
import type { ResourceIconName } from '../../resource-contract'
import { Button } from '@wizard-archive/ui/shadcn/components/button'
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from '@wizard-archive/ui/shadcn/components/popover'
import { getAvailableIconNames, getIconByName } from '../item-icons'
import { cn } from '@wizard-archive/ui/shadcn/lib/utils'

interface IconPickerProps {
  value: ResourceIconName | undefined
  onChange: (iconName: ResourceIconName | null) => void
  defaultIcon?: ResourceIconName
  triggerLabelledBy?: string
}

export function IconPicker({ value, onChange, defaultIcon, triggerLabelledBy }: IconPickerProps) {
  const [open, setOpen] = useState(false)
  const iconNames = getAvailableIconNames()
  const CurrentIcon = getIconByName(value ?? defaultIcon)

  return (
    <div className="h-9 w-9 flex-shrink-0">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger
          nativeButton
          render={
            <Button
              variant="outline"
              size="sm"
              className="h-9 w-9 p-0 min-w-9"
              aria-label={triggerLabelledBy ? undefined : 'Select icon'}
              aria-labelledby={triggerLabelledBy}
            >
              <CurrentIcon className="h-4 w-4" />
            </Button>
          }
        />
        <PopoverContent className="w-64 p-2" align="start" sideOffset={4}>
          <div className="grid grid-cols-6 gap-1">
            {iconNames.map((iconName) => {
              const IconComponent = getIconByName(iconName)
              const isSelected = value === iconName
              const isDefault = !value && iconName === defaultIcon

              return (
                <button
                  key={iconName}
                  type="button"
                  aria-label={`Select ${iconName} icon`}
                  aria-pressed={isSelected || isDefault}
                  className={cn(
                    'relative flex h-8 w-8 items-center justify-center rounded-md',
                    'hover:bg-accent hover:text-accent-foreground',
                    (isSelected || isDefault) && 'bg-accent ring-2 ring-ring',
                  )}
                  onClick={() => {
                    onChange(iconName === defaultIcon ? null : iconName)
                    setOpen(false)
                  }}
                  title={iconName}
                >
                  <IconComponent className="h-4 w-4" />
                </button>
              )
            })}
          </div>
        </PopoverContent>
      </Popover>
    </div>
  )
}
