...
function buildHeadingTree(headings: Array<Heading>): Array<HeadingNode> {
  ...
  for (const heading of headings) {
    const node: HeadingNode = {
      noteBlockId: heading.noteBlockId,
      text: heading.text,
      level: heading.level,
      children: [],
    }
...
function HeadingItem({
  ...
  node: HeadingNode
  depth: number
  onNavigate: (noteBlockId: NoteBlockId) => void
}) {
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          type="button"
          className="min-w-0 flex-1 truncate text-left text-sm rounded focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none"
          onClick={() => onNavigate(node.noteBlockId)}
        >
          {node.text}
        </button>
      </div>
...
export function OutlinePanel({
  ...
  onNavigate: (noteBlockId: NoteBlockId) => void
  state: RightSidebarOutlineState
}) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        ...
        <p className="text-sm font-medium text-muted-foreground">
          {state.availabilityState.message}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <ScrollArea className="flex-1 min-h-0">
        <div className="py-1">
          {tree.map((node) => (
            <HeadingItem key={node.noteBlockId} node={node} depth={0} onNavigate={onNavigate} />
          ))}
        </div>
      </ScrollArea>
    </div>
