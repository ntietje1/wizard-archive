...
} from '@wizard-archive/ui/shadcn/components/color-picker'
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
import { CheckerboardSwatch } from './checkerboard-swatch'
...
export function ColorPickerPopover({
  ...
  if (disabled !== openState.disabled) {
    setOpenState({
      disabled,
      open: disabled ? false : openState.open,
    })
  }
  const open = disabled ? false : openState.open
  ...
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn('Failed to build opacity gradient for color picker', {
        normalizedValue,
        error,
      })
    }
      ...
      >
        <CheckerboardSwatch className="h-full w-full rounded-sm">
          <span
            ...
            style={{
              backgroundColor: mixed ? 'transparent' : normalizedValue,
