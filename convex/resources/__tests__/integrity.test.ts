...
describe('resource integrity diagnostics', () => {
  ...
  it('recognizes only the published preview derivative as a current resource asset', async () => {
    const campaignUuid = generateDomainId(DOMAIN_ID_KIND.campaign)
    const resourceUuid = generateDomainId(DOMAIN_ID_KIND.resource)
    const assetUuid = generateDomainId(DOMAIN_ID_KIND.asset)
    const fixture = await t.run(async (ctx) => {
      ...
      await ctx.db.insert('resourceAssetOwners', { campaignUuid, resourceUuid, assetUuid })
      const publicationId = await ctx.db.insert('resourcePreviewPublications', {
        campaignUuid,
        resourceUuid,
        publication: {
          assetUuid,
    ...
    expect((await diagnose()).issues).not.toContainEqual(
      expect.objectContaining({ resourceUuid, assetUuid }),
    )
    await t.run((ctx) => ctx.db.delete(fixture.publicationId))
    expect((await diagnose()).issues).toContainEqual(
      expect.objectContaining({ resourceUuid, assetUuid }),
    )
  })
})
