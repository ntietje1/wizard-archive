...
import { addTagToBlockHandler, getBlockLevelTags, removeTagFromBlockHandler } from "../tags/tags"
...
export const addShareBlock = mutation({
    ...
    handler: async (ctx, args): Promise<string> => {
        ...
        if (currentSession) {
            await addTagToBlockHandler(ctx, args.noteId, args.blockId, currentSession.tagId)
        }

      return await addTagToBlockHandler(ctx, args.noteId, args.blockId, share.tagId)
    }
  })
    ...
    handler: async (ctx, args): Promise<string> => {
      ...
      if (currentSession) {
        ...
        if (hasCurrentSessionTag) {
            await removeTagFromBlockHandler(ctx, args.noteId, args.blockId, currentSession.tagId)
        }
      }
      return await removeTagFromBlockHandler(ctx, args.noteId, args.blockId, share.tagId)
    },
  })
